#修改在大厅的且距离-21 23 6的按钮最近玩家的计分项
    #修改类型为C类
    execute positioned -21 23 6 as @p[scores={class=0}] at @s run scoreboard players set @s class 3

    #修改难度为1
    execute positioned -21 23 6 as @p[scores={grade=0}] at @s run scoreboard players set @s grade 3

#清除进入时的玩家物品
execute positioned -21 23 6 as @p[scores={class=3,grade=3}] at @s run clear @s
