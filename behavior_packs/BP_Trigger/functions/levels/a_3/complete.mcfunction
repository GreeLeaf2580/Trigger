#所有玩家显示提示信息
tellraw @a {"rawtext": [{"text":"[A3]已通过！"}]}

#播放通关的音效
    #播放延迟时间
    execute unless block -17 25 7 concrete_powder ["color"="gray"]run scoreboard players set soundPlayer time 3
...
#开启接下来的关卡，修改大厅关卡显示
    ...
    #显示通过该类型
    execute if block -17 25 7 concrete_powder ["color"="gray"] run function levels/a_3/cheer